...
/**
 * Profilo dell'account coach loggato (specchio di AdminProfileView): dati
 * account con stato di approvazione, anagrafica fatturazione (riusa la card
 * own-row `BillingProfileForm` — serve per le commissioni), cambio password
 * e uscita. Il profilo ATLETA non si gestisce da qui: vive nelle schede
          ...
          <dl className="mt-4 space-y-3">
            {accountRows.map((row) => (
              <div key={row.label} className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
                ...
                <dd className="text-sm font-medium text-gray-200">{row.value}</dd>
              </div>
            ))}
          </dl>
        )}
