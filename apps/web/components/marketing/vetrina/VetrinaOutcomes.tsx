...
export async function VetrinaOutcomes() {
  ...
  const units = [
    {
      ...
      visual: <TrainingChart className="h-full" labels={{ trainingTitle: t("g.trainingTitle"), power: t("g.power"), hr: t("g.hr") }} />,
    },
    {
    ...
    },
    {
      ...
      visual: <PhysiologyChart className="h-full" labels={{ physioTitle: t("g.physioTitle"), lactate: t("g.lactate"), threshold: t("g.threshold"), power: t("g.power") }} />,
    },
    {
      title: "moduleTwinTitle",
      body: "moduleTwinBody",
      accent: "text-amber-300",
      visual: <MiniTwin className="h-full" labels={{ twinTitle: t("g.twinTitle"), readiness: t("g.readiness"), load: t("g.load"), recovery: t("g.recovery") }} />,
    },
  ];
  ...
  return (
    <section className="mt-28">
      ...
      <div className="mx-auto mt-14 grid max-w-5xl items-stretch gap-x-8 gap-y-14 md:grid-cols-2">
        {units.map((u, i) => (
          <Reveal key={u.title} delay={(i % 2) * 90} className="h-full">
            <div className="flex h-full flex-col">
              <h3 className={`text-lg font-black tracking-tight ${u.accent}`}>{t(u.title)}</h3>
              <p className="mb-5 mt-1.5 min-h-[2.75rem] max-w-md text-sm leading-relaxed text-gray-400">{t(u.body)}</p>
              <div className="flex-1">{u.visual}</div>
            </div>
          </Reveal>
        ))}
      </div>
