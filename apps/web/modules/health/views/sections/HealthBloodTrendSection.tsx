...
export function HealthBloodTrendSection({
  ...
  const t = useTranslations("HealthBloodTrendSection");
  return (
    <div className="rounded-2xl border border-rose-500/25 bg-gradient-to-br from-rose-950/[0.14] via-pink-950/[0.08] to-black/85 p-5 shadow-inner">
      <p className="font-mono text-[0.65rem] font-bold uppercase tracking-[0.2em] text-rose-400">{t("title")}</p>
      <p className="mt-1 text-sm text-gray-400">
        {t("subtitle")} {usingDemoTrend ? t("demoSuffix") : ""}
      </p>
      <div className="mt-4 h-[260px] w-full sm:h-[300px] lg:h-[320px]">
        {data.length === 0 ? (
          <p className="flex h-full items-center justify-center px-4 text-center text-sm text-gray-400">
            {!hasLatestStructuredRow
              ? t("importReportPrefix")
              : t("addSecondReport")}
            <code className="mx-1 text-rose-300">blood</code>
            {!hasLatestStructuredRow ? t("importReportSuffix") : ""}
          </p>
        ) : realPointCount === 1 && !usingDemoTrend ? (
          <p className="flex h-full items-center justify-center px-4 text-center text-sm text-gray-400">
            {t("singleReportNotice")}
          </p>
        ) : (
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              ...
              <Legend />
              <Line type="monotone" dataKey="emoglobina" name={t("hemoglobinLine")} stroke={CHART_SIGNAL.hr} strokeWidth={CHART_STROKE.base} dot />
              <Line type="monotone" dataKey="ferritina" name={t("ferritinLine")} stroke={CHART_SIGNAL.power} strokeWidth={CHART_STROKE.base} dot />
              ...
              <Line type="monotone" dataKey="glicemia" name={t("glucoseLine")} stroke={CHART_SIGNAL.glucose} strokeWidth={CHART_STROKE.base} dot />
            </LineChart>
          </ResponsiveContainer>
        )}
      </div>
