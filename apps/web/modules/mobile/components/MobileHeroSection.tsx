...
export function MobileHeroSection({ score, label }: MobileHeroSectionProps) {
  const t = useTranslations("MobileHeroSection");
  return (
    <section className="relative flex items-end justify-between gap-4">
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-2 text-xs text-gray-400">{t("systemTagline")}</p>
      </div>
      <div className="shrink-0 rounded-2xl border border-white/10 bg-black/30 px-3 py-3">
        <div className="text-[0.55rem] uppercase tracking-wider text-gray-500">{t("readinessScore")}</div>
        <div className="mt-1 flex items-center gap-2">
          <DashboardReadinessRing score={score} size="sm" />
          <div>
            <div className="text-sm font-semibold capitalize text-white">{label ?? t("waiting")}</div>
          </div>
        </div>
      </div>
    </section>
