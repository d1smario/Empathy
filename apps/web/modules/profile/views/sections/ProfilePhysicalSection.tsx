...
export function ProfilePhysicalSection({ form, setForm }: ProfilePhysicalSectionProps) {
  const t = useTranslations("ProfilePhysicalSection");
  return (
    <div>
      <h3 className={`profile-section-band tone-${profileToneForEditorSection("physical")}`}><span className="profile-kpi-dot" />{t("physicalMeasurementsTitle")}</h3>
      ...
      <div className="form-group"><label className="form-label">{t("restingHr")}</label><input className="form-input" type="number" value={form.resting_hr_bpm} onChange={(e) => setForm((f) => ({ ...f, resting_hr_bpm: e.target.value }))} /></div>
      <div className="form-group"><label className="form-label">{t("maxHr")}</label><input className="form-input" type="number" value={form.max_hr_bpm} onChange={(e) => setForm((f) => ({ ...f, max_hr_bpm: e.target.value }))} /></div>
      <div className="form-group"><label className="form-label">{t("thresholdHr")}</label><input className="form-input" type="number" value={form.threshold_hr_bpm} onChange={(e) => setForm((f) => ({ ...f, threshold_hr_bpm: e.target.value }))} /></div>
      </div>
    </div>
  );
}
