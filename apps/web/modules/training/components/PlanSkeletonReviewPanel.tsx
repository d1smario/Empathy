...
/** Patch di revisione per-settimana: gli UNICI campi che il coach corregge inline. */
...
function WeekReviewRow({
  ...
  }, [hours, notes, primary, onSave, week.id]);
  ...
  return (
    <tr className="border-b border-white/[0.05] align-middle">
      ...
      <td className="p-2 font-mono text-xs tabular-nums text-orange-200">{week.loadTarget}</td>
                ...
                <tbody>
                  {skeleton.mesocycles.map((meso) => (
                    <tr key={meso.id} className="border-b border-white/[0.05]">
                      ...
                      <td className="p-2 font-mono tabular-nums">{meso.weeks}</td>
