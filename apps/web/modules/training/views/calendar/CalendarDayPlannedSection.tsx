...
export function CalendarDayPlannedSection({
  ...
  const t = useTranslations("CalendarDayPlannedSection");
  return (
    <div id="calendar-day-planned-detail" className="mb-8 scroll-mt-24 w-full min-w-0">
      <Pro2SectionCard
        accent="orange"
        title={t("title")}
        subtitle={t("subtitle", { selectedDate, count: dayPlanned.length })}
        icon={CalendarDays}
      >
        {dayPlanned.length >= 2 && athleteId ? (
          <div className="mb-4 rounded-xl border border-amber-400/35 bg-amber-950/30 px-3 py-3">
            <p className="text-xs leading-relaxed text-amber-100/95">
              {t.rich("multipleWarning", {
                count: dayPlanned.length,
            ...
            {dayDeleteAllConfirm ? (
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                >
                  {dayDeleteAllBusy ? t("deleting") : t("confirmDeleteAll", { count: dayPlanned.length })}
                </button>
                <button
                  type="button"
                  disabled={dayDeleteAllBusy}
