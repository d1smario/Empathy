...
// NB: il secret su Supabase è salvato come `STRIPE-SECRET-KEY` (trattini, non
// rinominabile da dashboard). Leggiamo prima il nome canonico con underscore,
// poi il fallback coi trattini così funziona con l'attuale segreto.
const STRIPE_SECRET_KEY = Deno.env.get("STRIPE_SECRET_KEY") ?? Deno.env.get("STRIPE-SECRET-KEY") ?? "";
...
Deno.serve(async (req: Request) => {
  ...
  // Prova PER-PRODOTTO: `duration_days` su un prodotto ricorrente = giorni di prova gratuita
  // prima del primo addebito → la subscription Stripe parte in stato 'trialing' (poi si
  // rinnova/addebita da sola). Assente o 0 → nessuna prova (addebito immediato). Clamp Stripe 1..730.
  if (mode === "subscription" && base.duration_days != null && Number(base.duration_days) > 0) {
    const trialDays = Math.min(730, Math.max(1, Math.round(Number(base.duration_days))));
